        ...
        _initialize: function () {
            if (!initByName) {
                ...
                for (let index in _GRAPH_ATTRIBUTES) {
                    ...
                    lines.push(`                    GRAPH_ATTRYBUTES[${index}] = (${_GRAPH_ATTRIBUTES[index].setValue})(graph)`)
                }
                lines.push(
    ...
    const helper = {
        ...
        export() {
            ...
            function ${STATIC_PARAMETER.USE_FUNCTION.toLowerCase()}(graph, params, callback) {
                ...
                const graphAttributes = (${_private._initialize()})(_graph)
                const indexArrays = (${_private._prepare()})(_graph)
                if (parameters.useWorker) {
                    const workerFunc = function () {
                        addEventListener('message', (e) => {
                            let { initOGDF, graphAttributes, indexArrays, originalParameters } = JSON.parse(e.data)
